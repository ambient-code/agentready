...
2. **TestExecutionAssessor: Check test organization.** Look for evidence of
   unit/integration test separation (separate directories, markers like
   `@pytest.mark.integration`, test filtering in Makefile/scripts). Note:
   partially feasible using rules; heuristics are language-specific and
   incomplete.
