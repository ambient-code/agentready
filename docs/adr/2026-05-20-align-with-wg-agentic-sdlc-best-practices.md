...
|---|---|---|---|---|---|
| 1 | **Context file length validation** | T1 | Partial | Feasible using rules. Count lines. | CLAUDEmdAssessor checks existence/sections but does NOT check length. ProgressiveDisclosureAssessor checks root <150 lines, but only for repos >50K LOC. The BP says <150 lines recommended, hard cap 300, for ALL repos. Scoring: full credit for <=150 lines, partial credit for <=300 lines, no credit for >300 lines. |
...
1. **CLAUDEmdAssessor: Add length validation.** Check context file line count
   for all repos. Full credit for <=150 lines, partial credit for <=300 lines,
   no credit for >300 lines (the BP hard cap). Currently only
